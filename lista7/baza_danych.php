...
class BazaDanych {
    ...
    public function logowanie($login, $haslo) {
        ...
        $sql = "select login, id from uzytkownik where login = '" . $login . "' and haslo = md5('" . $haslo . "')"; //SQL injection
    ...
    public function ustawNoweHaslo($haslo, $hash) {
        ...
        if ($line = mysqli_fetch_array($result)) {
            ...
            $sql = "update uzytkownik set haslo = md5('" . $haslo . "') where id = " . $uzytkownik_id; //SQL injection
            mysqli_query($conn, $sql);
    ...
    public function listaNiepotwierdzonych() {
        $wynik = array();
        $conn = new mysqli(DB_HOST, DB_USER, DB_PASSWORD, DB_NAME);
        if ($conn->connect_error) {
            die("Connection failed: " . $conn->connect_error);
        }
        $sql = "select id, data, uzytkownik_id, nrb_odbiorcy, kwota from przelew where status = 0";
        ...
        while ($line = mysqli_fetch_array($result)) {
            ...
            $rekord[0] = $line["id"];
            $rekord[1] = $line["data"];
            $rekord[2] = $line["uzytkownik_id"];
            $rekord[3] = $line["nrb_odbiorcy"];
            $rekord[4] = $line["kwota"];
            $wynik[] = $rekord;
        }
    ...
    public function listaPrzelewowSzukaj($nrb) {
        $wynik = array();
        $conn = new mysqli(DB_HOST, DB_USER, DB_PASSWORD, DB_NAME);
        if ($conn->connect_error) {
            die("Connection failed: " . $conn->connect_error);
        ...
        $sql = "select data, nrb_odbiorcy, kwota from przelew where uzytkownik_id = " . $_SESSION["uzytkownik_id"] . " "
                . "and status > 0 and nrb_odbiorcy = '" . $nrb . "'";
    ...
    public function dodajPrzelew($nrb, $kwota) {
        ...
        $sql = "insert into przelew(uzytkownik_id, nrb_odbiorcy, kwota, hash, status, data) "
                . "values(" . $_SESSION["uzytkownik_id"] . ", '" . $nrb . "', " //SQL injection
    ...
    public function zatwierdzPrzelewAdmin($id) {
        $_SESSION['hash'] = md5(time() . rand(0, 100));
        $conn = new mysqli(DB_HOST, DB_USER, DB_PASSWORD, DB_NAME);
        if ($conn->connect_error) {
            die("Connection failed: " . $conn->connect_error);
        }
        $sql = "update przelew set status = 1 where id = " . $id;
        $conn->query($sql) or die(mysqli_error($conn));
        $conn->close();
        $_SESSION["komunikat"] = "zatwierdzono przelew";
    }
